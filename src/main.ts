...
async function discoverBrowserWallets(): Promise<EIP6963ProviderDetail[]> {
  ...
  // Rainbow, Rabby, etc.) that inject window.ethereum but don't announce via EIP-6963.
...
renderRoute();
setBridgeButtonState();  const handle = (event: WindowEventMap["eip6963:announceProvider"]) => {
    providers.set(event.detail.info.uuid, event.detail);
  };
...
async function runBridge() {
  ...
  try {
    ...
    console.error(err);
    setLogStatus("error", "error");
    setHint(err?.message ?? "Something went wrong during the bridge.", "error");
  } finally {
    railPulse.classList.remove("running");
    connectBtn.disabled = false;
    switchBtn.disabled = false;
